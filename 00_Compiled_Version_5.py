...
class Game:
    ...
    def reveal_boxes(self):
        ...
        # Update game_statistics_list with current balance
        # (This means that the item in position 1 of the list is replaced with the current balance)
    ...
    # Allows for the help window to be called
    def help(self):
        print("You asked for help")
        get_help = Help(self)

    # Allows for the statistics window to be called
...
class GameStatistics:
    ...
    def __init__(self, partner, game_history, game_statistics):
        ...
        self.starting_balance_value_label = Label(self.details_frame, font=content,
                                                    text="${}.00".format(game_statistics[0]),
                                                    anchor="w")
        ...
        self.current_balance_value_label = Label(self.details_frame, font=content,
                                                    text="${}.00".format(game_statistics[1]),
                                                    anchor="w")
...
class Export:
    ...
    def save_history(self, partner, game_history, game_statistics):
        ...
        # Prints Game Statistics for testing purposes
        print("Game Statistics", game_statistics)
        ...
        if has_error == "yes":
            ...
            # (Star character from the following address: https://unicode-table.com/en/2605/)
            f.write("\n★★★ Game Statistics ★★★\n\n")

            # Game Statistics

            # Finds and creates variables for the 
            # ... starting and closing funds generated by the game
            start_funds = game_statistics[0]
            closing_funds = game_statistics[1]
            
            # Writes Starting and Closing Balances to the export file
            f.write("Starting Balance: ${:.2f}\n".format(start_funds))
            f.write("Closing Balance: ${:.2f}\n".format(closing_funds))

            # To find whether a win or loss has occured, and calculates amount
            if game_statistics[1] > game_statistics[0]:
                export_win_or_loss = "Amount Won:"
                export_amount = game_statistics[1] - game_statistics[0]
            else:
                export_win_or_loss = "Amount Lost:"
                export_amount = game_statistics[0] - game_statistics[1]

            # Writes the amount won or lost to the export file
            f.write("{} ${:.2f}\n".format(export_win_or_loss, export_amount))
